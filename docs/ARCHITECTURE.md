...
7. Ввод: мышь и клавиатура доходят до окон обоев напрямую, без хуков перехвата всего ввода
   (`SetWindowsHookEx` и т. п.), так как это настоящие (хоть и невидимые) окна в дереве рабочего
   стола. Отдельно от этого один глобальный хоткей push-to-talk (`RegisterHotKey`, планер.hotkey,
   `Hotkeys.cs`) зарегистрирован для голосового ввода планера — он ловит только своё сочетание
   клавиш, а не весь ввод. Правый клик по-прежнему открывает
